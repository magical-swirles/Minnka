...
"""
def break_ties(arbitrarily_chosen, tests, tiebreak_map): # breaks ties between tests based on tiebreak_map
    if len(tiebreak_map) == 0:
        # print("[break_ties] No tiebreak_map")
    ...
    for test in tests:
        if (tiebreak_map[test] < smallest_value):
            ...
            chosen_test = test
    # print("[break_ties] Chosen test: " + chosen_test + " value: " + str(smallest_value))
    return chosen_test
...
def get_best_test(mapping, tiebreak_map):
    ...
    if len(coverage_mapping[max_coverage]) > 1:
        ...
        # print("[get_best_test] Max coverage: " + str(max_coverage) + " Tie between the tests: " + str(coverage_mapping[max_coverage]) + " Chose test: " + current)
    return current
...
def find_essential(entity_to_tests, mapping):
    ...
    for entity in entity_to_tests.keys():
        ...
        if len(entity_to_tests[entity]) == 1:
            ...
            essential_tests.add(test + " is essential for " + entity + ", also covers " + str(mapping[test]))
...
"""
def remove_redundant(mapping, tiebreak_map):
...
def gre(mapping, tiebreak_map, percentage=1.0):
    ...
    while len(covered_entities) < percentage * len(total_entities):
        mapping = remove_redundant(mapping, tiebreak_map)
        entity_to_test = inverse_dict(mapping)
...
def hgs_select_test(size, cardinality, max_size, tiebreak_map):
    ...
    if len(tests) > 1:
        return break_ties(tests[0], tests, tiebreak_map)
        # print("[hgs_select_test] Cardinality: " + str(size) + " Tie between the tests: " + str(tests) + " Chose: " + str(tests[0]))
    return tests[0]
...
def hgs(st_to_test, tiebreak_map, percentage=1.0):
    ...
    covered_entities = set()
    print(test_to_st.values())
    if len(test_to_st.values()) == 0:
        return reduced_testsuite
...
def reduce_suite(data_file, orig_file, algorithm, out, tiebreak_map, percentage=1.0):
    ...
    if (out == "STDOUT"):
        ...
        with open(out, "w") as out:
            for test in selected_tests:
                out.write(test + '\n')
    print("[reduce_suite] Number of selected tests: " + str(len(selected_tests)))
...
def main(args):
    if len(args) != 6 and len(args) != 7:
        print ('Please provide 5 arguments: coverage file, original tests file, algorithm, output_file, tiebreak_file')
        print ('If output_file is STDOUT then the output will be written to standard output')
