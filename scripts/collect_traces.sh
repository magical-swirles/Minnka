...
# Usage: collect_traces.sh <project-directory> <project-name> <extension-directory> <mop-directory> <log-directory> [threads=20]
...
function collect_traces() {
  python3 ${SCRIPT_DIR}/collect_traces.py ${PROJECT_DIR} ${PROJECT_NAME} ${EXTENSION_DIR} ${MOP_DIR} ${LOG_DIR} ${THREADS:-20} ${TRACES_TIMEOUT:-20}
  status=$?
...
if [[ -z "$5" ]]; then
  echo "Usage: ./collect_traces.sh <project-directory> <project-name> <extension-directory> <mop-directory> <log-directory> [threads=20] [timeout=20min]"
  exit 1
fi
